...
/**
 * Extract audio using system yt-dlp (works better than node_modules version)
 */
const extractWithYtDlp = async (videoUrl, outputPath) => {
    console.log('[yt-dlp] Extracting audio from:', videoUrl);
    
    // Use system yt-dlp - it works!
    const command = `/usr/local/bin/yt-dlp -x --audio-format mp3 --audio-quality 0 \
        --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" \
...
const getMetadata = async (videoUrl) => {
    const command = `/usr/local/bin/yt-dlp --dump-json \
        --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" \
        --extractor-args "youtube:player_client=android" \
        --no-check-certificates \
        "${videoUrl}"`;
...
export const extractAndUploadAudio = async (userId, videoUrl) => {
    ...
    try {
        if (videoUrl.includes("tiktok.com")) {
            console.log(`[TikTok] Processing: ${videoUrl}`);
            const result = await tiktokdl.Downloader(videoUrl, { version: "v3" });

            if (result.status !== "success" || !result.result) {
                throw new Error(`TikTok API error: ${result.message || "Failed"}`);
            }

            title = result.result.description || "TikTok Video";
            description = result.result.description || "No Description";
            thumbnail = result.result.origin_cover || result.result.cover || null;
            creatorAttribution = result.result.author?.nickname || result.result.author?.unique_id || null;

            console.log(`[TikTok] Title: ${title}`);

            const audioUrl = result.result.music || result.result.audio;

            if (audioUrl) {
                const finalAudioUrl = Array.isArray(audioUrl) ? audioUrl[0] : audioUrl;
                ...
                const videoUrlDirect = result.result.videoHD || result.result.videoSD;
                if (!videoUrlDirect) throw new Error("No media URL found");
                
                console.log(`[TikTok] Extracting from video...`);
                await extractWithYtDlp(videoUrlDirect, tempFilePath);
            }

        } else {
            // YouTube/Instagram
            console.log(`[Social] Processing: ${videoUrl}`);
            ...
            if (metadata) {
                ...
                creatorAttribution = metadata.uploader || metadata.channel || null;
                console.log(`[Metadata] Title: ${title}`);
            }

            // Extract audio
            await extractWithYtDlp(videoUrl, tempFilePath);
        }

        // Verify file
        if (!fs.existsSync(tempFilePath)) {
            throw new Error("Audio file not created - check if ffmpeg is installed");
        }
        ...
        console.log(`[Audio] File created: ${(fileStats.size / 1024 / 1024).toFixed(2)} MB`);
